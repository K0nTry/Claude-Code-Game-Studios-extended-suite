...
user-invocable: true
argument-hint: "<book-path> [--out ./game-project]"
---
...
```bash
/book2game "C:/path/to/book.pdf" --out "./my-game"
/book2game "./book.epub"
/book2game "./book.docx" --out "C:/Users/Kon_Try/Desktop/my-game"
```
...
```bash
python "${CLAUDE_SKILL_DIR}/scripts/parse_book.py" "$BOOK_PATH" --out "$OUT_DIR" --stop-after-entities
```

Το `--stop-after-entities` σταματάει το script **αμέσως μετά** το draft `entities.json`, πριν ξεκινήσει το Βήμα 2 (12-Cycle engine) — αυτό είναι απαραίτητο ώστε ο agent να προλάβει να βελτιώσει το `entities.json` πριν τροφοδοτηθεί στα βαριά downstream modules. Χωρίς αυτό το flag, `parse_book.py` τρέχει Βήμα 1+2 σε μία αδιάσπαστη κλήση και δεν υπάρχει σημείο παρέμβασης.

Γράφει (και σταματάει εκεί):
...
- `entities.json` (αυτόνομη εξαγωγή μέσω `extract_entities.py` — draft, regex-based).

### Βήμα 1.5 — Agent Entity Refinement

Το `entities.json` του Βήματος 1 είναι regex-based πρόχειρο (γρήγορο, δωρεάν, αλλά υπερ-εξάγει: μπερδεύει τυχαίες κεφαλαιοποιημένες λέξεις με πρόσωπα και δεν ενώνει ψευδώνυμα). Πριν το Βήμα 2, ο agent το βελτιώνει:

1. Διαβάζει τα `chapters/*.md` σε παρτίδες, με παράλληλα subagents όταν το βιβλίο έχει πολλά κεφάλαια (ώστε να μη χρειάζεται να διαβαστεί όλο το βιβλίο σε ένα context).
2. Για κάθε παρτίδα εντοπίζει τους **πραγματικούς** χαρακτήρες/τοποθεσίες/αντικείμενα/έννοιες/σχέσεις/φατρίες, με νόημα — ενώνει ψευδώνυμα και παραλλαγές ονόματος στην ίδια οντότητα (π.χ. "Neuromancer" και "Neuro"), αγνοεί κεφαλαιοποιημένες λέξεις που δεν είναι πρόσωπα/τόποι.
3. Χρησιμοποιεί το regex `entities.json` **μόνο σαν λίστα υποψηφίων-υπενθύμιση** (να μη ξεχαστεί κάποιος πραγματικός χαρακτήρας) — όχι σαν αλήθεια.
4. Γράφει την έξοδο **στο ίδιο ακριβώς σχήμα** που ήδη περιμένει ο υπόλοιπος κώδικας: dict με keys `characters, locations, objects, concepts, relationships, factions, timeline, stats`. Κάθε character έχει `name/role/traits/evidence:[{quote,line}]`. Κάθε relationship έχει `source/target/relation/type/evidence`.
5. Προσθέτει `confidence` (0-1) και `method: "agent-semantic-extraction"` σε κάθε entity.
6. Αντικαθιστά (overwrite) το `$OUT_DIR/entities.json` με τη βελτιωμένη έκδοση, **ίδιο path, ίδιο σχήμα**.
7. Συνεχίζει στο Βήμα 2:

```bash
python "${CLAUDE_SKILL_DIR}/scripts/parse_book.py" "$BOOK_PATH" --out "$OUT_DIR" --resume-entities
```

Το `--resume-entities` **δεν ξαναγράφει** το `entities.json` (ούτε ξανακάνει extraction/chunking — τα διαβάζει από τα ήδη-γραμμένα `full_text.md`/`chapters/`) — απλά συνεχίζει κατευθείαν στο Βήμα 2 με ό,τι entities.json βρει στον δίσκο. Χρειάζεται το ίδιο `$BOOK_PATH` (για τα metadata) και το ίδιο `--out`.
